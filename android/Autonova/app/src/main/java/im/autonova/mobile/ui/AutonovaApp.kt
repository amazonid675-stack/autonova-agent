...
@Composable private fun FilesAndStorageScreen(viewModel: AutonovaViewModel) {
    ...
    LocalStorageActionConfirmation(pendingAction, noteTitle, { storage.createNote(noteTitle, noteContent); viewModel.recordDeviceAction("storage.create", "User confirmed creating a note in the selected folder."); noteTitle = ""; noteContent = ""; localFiles = storage.listFiles(); pendingAction = null }, { document -> runCatching { storage.open(document) }; viewModel.recordDeviceAction("storage.open", "User confirmed opening ${document.name}."); pendingAction = null }, { document -> runCatching { storage.share(document) }; viewModel.recordDeviceAction("storage.share", "User confirmed sharing ${document.name}."); pendingAction = null }, { document -> storage.delete(document); viewModel.recordDeviceAction("storage.delete", "User confirmed deleting ${document.name}."); localFiles = storage.listFiles(); pendingAction = null }, { pendingAction = null })
}
