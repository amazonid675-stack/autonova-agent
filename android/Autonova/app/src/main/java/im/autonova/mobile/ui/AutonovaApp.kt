...
@Composable private fun PageHeader(kicker: String, title: String, detail: String? = null) {
    Row(verticalAlignment = Alignment.CenterVertically) { Icon(painter = painterResource(R.drawable.ic_autonova_mark), contentDescription = "Autonova logo", tint = Color.Unspecified, modifier = Modifier.height(24.dp).width(24.dp)); Spacer(Modifier.width(8.dp)); Text(kicker.uppercase(), color = Lavender, style = MaterialTheme.typography.labelSmall) }
...
@Composable private fun CommandScreen(viewModel: AutonovaViewModel, modifier: Modifier) {
    ...
    Column(modifier.fillMaxSize().imePadding().padding(horizontal = 16.dp, vertical = 12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Text(if (configured) "Your prompt will use the connected optional agent. If it cannot reply, the saved request remains visible with recovery guidance." else "Your prompt is saved locally and receives an immediate local working response. Import a model for generated offline answers.", color = Muted, style = MaterialTheme.typography.bodySmall)
...
@Composable private fun ColumnScope.LearningScreen(viewModel: AutonovaViewModel) {
    val candidates by viewModel.learningCandidates.collectAsState(); val grants by viewModel.capabilityGrants.collectAsState(); val improvements by viewModel.improvements.collectAsState(); val configured by viewModel.configured.collectAsState(); var title by remember { mutableStateOf("") }; var content by remember { mutableStateOf("") }; var capability by remember { mutableStateOf("web.research") }; var scope by remember { mutableStateOf("") }; var rationale by remember { mutableStateOf("") }; var improvementScope by remember { mutableStateOf("WORKFLOW") }; var improvementTitle by remember { mutableStateOf("") }; var improvementChange by remember { mutableStateOf("") }; var improvementEvidence by remember { mutableStateOf("") }; var improvementTest by remember { mutableStateOf("") }; var improvementBenchmark by remember { mutableStateOf("") }; var improvementVersion by remember { mutableStateOf("1.0") }
    Column(Modifier.fillMaxWidth().weight(1f).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
...
@Composable private fun ColumnScope.DeviceCapabilitiesScreen(viewModel: AutonovaViewModel) {
    ...
    if (configured) pendingCloudFallback?.let { prompt -> ConfirmDialog("Use cloud fallback?", "Your local model could not complete this request. Send this prompt to your connected cloud agent instead?\n\n${prompt.take(300)}", "Use cloud", { viewModel.confirmCloudFallback() }, { viewModel.dismissCloudFallback() }) }
}
...
@Composable private fun FilesAndStorageScreen(viewModel: AutonovaViewModel) {
    val remoteFiles by viewModel.files.collectAsState(); val localKnowledgeUsage by viewModel.localKnowledgeUsage.collectAsState(); val configured by viewModel.configured.collectAsState(); val context = LocalContext.current; val storage = remember(context) { DeviceStorage(context, SecureConfig(context)) }
    ...
    if (storage.hasFolder()) {
        ...
        visibleFiles.forEach { document -> SurfaceCard { Text(document.name, fontWeight = FontWeight.SemiBold); Text(if (document.isDirectory) "Local workspace folder" else "${document.mimeType} · ${document.sizeBytes} bytes", color = Muted, style = MaterialTheme.typography.bodySmall); if (!document.isDirectory) FilesAndStorageActionControls(document, true, { pendingAction = it }, onEdit = if (storage.isEditable(document)) { { pendingAction = LocalStorageAction.Edit(document) } } else null, onExport = { pendingAction = LocalStorageAction.ExportCopy(document) }, onArchive = { pendingAction = LocalStorageAction.Archive(document) }, onUpload = { storage.readBytes(document)?.let { if (configured) { viewModel.recordDeviceAction("storage.upload", "User selected ${document.name} for upload to the protected workspace."); viewModel.uploadLocalFile(document, it) } else viewModel.showError("Upload needs a connected optional agent. Local editing and indexing remain available now.") } }, onIndex = { storage.readBytes(document)?.let { viewModel.recordDeviceAction("document.local_index", "User chose ${document.name} for local knowledge indexing."); viewModel.indexLocalDocument(document, it) } }) else { Text("Folders can be searched, exported as a copy, or archived locally. Android does not expose unrestricted shell execution for them.", color = Muted, style = MaterialTheme.typography.bodySmall); Row { TextButton(onClick = { pendingAction = LocalStorageAction.ExportCopy(document) }) { Text("Export workspace copy") }; TextButton(onClick = { pendingAction = LocalStorageAction.Archive(document) }) { Text("Archive workspace") } } } } }
...
@Composable private fun GitHubScreen(viewModel: AutonovaViewModel) {
    val github by viewModel.github.collectAsState(); val connection by viewModel.githubConnection.collectAsState(); val operations by viewModel.githubOperations.collectAsState(); val configured by viewModel.configured.collectAsState(); val context = LocalContext.current; val workspaceStorage = remember(context) { DeviceStorage(context, SecureConfig(context)) }; var workspaceFiles by remember { mutableStateOf(workspaceStorage.searchFiles("").filter { workspaceStorage.isEditable(it) }) }; var selectedWorkspaceFile by remember { mutableStateOf<LocalDocument?>(null) }; var syncPath by remember { mutableStateOf("") }; var syncMessage by remember { mutableStateOf("Sync selected Autonova workspace file") }; var syncBranch by remember { mutableStateOf("") }; var repository by remember { mutableStateOf("") }; var token by remember { mutableStateOf("") }; var scopes by remember { mutableStateOf("Contents: read/write; Issues: read/write; Pull requests: read/write") }; var operation by remember { mutableStateOf("CREATE_ISSUE") }; var title by remember { mutableStateOf("") }; var body by remember { mutableStateOf("") }; var branch by remember { mutableStateOf("") }; var sourceBranch by remember { mutableStateOf("main") }; var head by remember { mutableStateOf("") }; var base by remember { mutableStateOf("main") }
    Text("GitHub context", style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold); Text(if (configured) "Inspect public repositories, then optionally connect a fine-grained token for the exact repositories and permissions you choose. Token material is encrypted server-side and never returned to Android." else "GitHub inspection and confirmed writes require your optional remote agent because this phone app never stores a GitHub token locally.", color = Muted, style = MaterialTheme.typography.bodySmall)
    Row(verticalAlignment = Alignment.CenterVertically) { OutlinedTextField(value = repository, onValueChange = { repository = it }, modifier = Modifier.weight(1f), label = { Text("owner/repository") }, singleLine = true); Spacer(Modifier.width(8.dp)); Button(onClick = { if (configured) viewModel.inspectGitHub(repository) else viewModel.showError("GitHub inspection needs a connected optional agent. Configure it in More → Settings.") }, enabled = repository.contains("/")) { Text(if (configured) "Inspect" else "Show setup") } }
