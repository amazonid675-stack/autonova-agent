...
class LocalFirstPrimaryWorkflowInstrumentedTest {
    @Test fun local_only_project_memory_and_task_creation_persist_without_a_remote_session() = runBlocking {
        ...
        assertEquals("Review the offline plan", withTimeout(3_000) { cache.observeTasks().first { items -> items.any { it.request == "Review the offline plan" } } }.first { it.request == "Review the offline plan" }.request)
        database.close()
    }

    @Test fun local_only_prompt_saves_the_request_and_an_honest_model_readiness_response() = runBlocking {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val config = SecureConfig(context)
        config.clearSession()
        config.clearLocalModel()
        ...
        assertTrue(messages.any { it.role == "assistant" && it.content.contains("Offline request saved locally") })
        database.close()
    }
}
